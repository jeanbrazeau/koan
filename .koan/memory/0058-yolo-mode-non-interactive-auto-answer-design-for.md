...
For `koan_yield`: when `AppState.yolo` is `True`, the blocking `asyncio.Future` wait on user input was skipped. The auto-response was computed by `_yolo_yield_response(suggestions)` using the following priority: (1) command text of the first suggestion with `recommended: True` and `id != "done"`, (2) command text of the first suggestion with `id != "done"`, (3) the string `"proceed"`. Leon selected this approach over a fixed return string, preferring that suggestion-driven responses kept the orchestrator on the workflow's intended path.
