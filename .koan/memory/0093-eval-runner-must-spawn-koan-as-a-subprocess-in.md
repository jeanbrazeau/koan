...
modified: '2026-04-23T16:53:59Z'
related:
- 0049-eval-solver-answers-all-koan-interactive-gates.md
- 0050-eval-benchmark-fixtures-are-manual-git-snapshots.md
...
Context. The previous runner design (entry 0049, 2026-04-19) flipped `app_state.yolo = True` on an in-process `AppState` handle and called `create_app()` from the same Python process that drove the eval. The orchestrator CLI (`claude` / `codex`) was spawned into that same process's file system view, with the fixture git submodule checked out at a pinned SHA (entry 0050) as the target repo.
